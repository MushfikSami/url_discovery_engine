...
def setup_database():
    """Creates the banglapedia_pages table if it doesn't exist."""
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS banglapedia_pages (
    ...
    cursor.close()
    conn.close()
    print("[+] Database table 'banglapedia_pages' verified/ready.")
...
def insert_pending_url(url):
    ...
    cursor = conn.cursor()
    cursor.execute("""
        INSERT INTO banglapedia_pages (url, status) 
        VALUES (%s, 'pending') 
...
def get_pending_urls():
    ...
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT url FROM banglapedia_pages WHERE status = 'pending';")
